var BookDetail = {
    ...
    FollowAuthor: function (authorId, operation) {
        $.ajax({
            type: "POST",
            url: "/user/followAuthor",
            data: JSON.stringify({'authorId': authorId, 'operation': operation}),
            dataType: "json",
            success: function (data) {
                if (data.code == 0) {
                    if (operation == 1) {
                        $("#followAuthor").removeClass("icon_qyzz");
                        $("#followAuthor").html("已关注");
                        $("#followAuthor").unbind('click').removeAttr('onclick').click(function(){ BookDetail.FollowAuthor(authorId, 0); });
                    } else {
                        $("#followAuthor").addClass("icon_qyzz");
                        $("#followAuthor").html("关注");
                        $("#followAuthor").unbind('click').removeAttr('onclick').click(function(){ BookDetail.FollowAuthor(authorId, 1); });
                    }
    ...
    SaveComment: function (cmtBId, cmtCId, cmtDetail) {
        ...
        console.log($.cookie('csrf_access_token'));
        $.ajax({
            type: "POST",
            url: "/book/addBookComment",
            data: JSON.stringify({'bookId': $("#bookId").val(), 'commentContent': cmtDetail}),
            dataType: "json",
            success: function (data) {
                if (data.code == 0) {
                    $('#txtComment').val("")
