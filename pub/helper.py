...
class ModelFilter:
    ...
    type_greater_equals = "greater_equals"
    type_less = "less"
    type_less_equals = "less_equals"
    type_vague = "vague"
    ...
    def greater_equals(self, field_name, value):
        """
        大于等于查询字段
        :param field_name: 模型字段名称
        :param value: 值
        """
        if value and value != '':
            self.filter_field[field_name] = {"data": value, "type": self.type_greater_equals}
    ...
    def less_equals(self, field_name, value):
        """
        小于等于查询字段
        :param field_name: 模型字段名称
        :param value: 值
        """
        if value and value != '':
            self.filter_field[field_name] = {"data": value, "type": self.type_less_equals}
    ...
    def get_filter(self, model: db.Model):
        ...
        for k, v in self.filter_field.items():
            ...
            if v.get("type") == self.type_greater:
                self.filter_list.append(getattr(model, k) > v.get("data"))
            if v.get("type") == self.type_greater_equals:
                self.filter_list.append(getattr(model, k) >= v.get("data"))
            if v.get("type") == self.type_less:
                self.filter_list.append(getattr(model, k) < v.get("data"))
            if v.get("type") == self.type_less_equals:
                self.filter_list.append(getattr(model, k) <= v.get("data"))
