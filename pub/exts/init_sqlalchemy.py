...
class Query(BaseQuery):
    ...
    @classmethod
    def __get_page_and_limit(cls, location) -> namedtuple:
        page = 1
        limit = 10
        total_count = None
        if 'query' == location:
            ...
            total_count = req_json.get('total_count')
        elif 'header' == location:
            page = request.headers.get('page', type=int)
            limit = request.headers.get('limit', type=int)
            total_count = request.headers.get('total_count', type=int)
    ...
    def layui_paginate_from_json(self):
        req_json = request.get_json(force=True)
        return self.paginate(page=req_json.get("page"),
                             per_page=req_json.get('limit'),
                             error_out=False)

    def layui_paginate_from_body(self, body: PageModel):
        return self.paginate(page=body.page,
                             per_page=body.limit,
                             error_out=False)

    def layui_paginate_from_query(self, query: PageModel):
        return self.paginate(page=query.page,
